# FORKED PROJECT OF https://github.com/imliam/vscode-inline-parameters

This demo extension adds inline parameter hints and hides specific source code parts, namely the following:

JavaScript:

- keywords of variable declarations (var, const, let, ...)

TypeScript:

- keywords of variable declarations (var, const, let, ...)
- types of variables and function parameters (number, string, boolean)
- return types of functions and arrow functions (void, number, string, boolean)

The hiding of the source code parts is done by the following two hacks:

- setting foreground color equal to background color
- setting letter spacing to -100em (negative)

## Running the sample

- `npm install`
- open the file `src/extension.ts`, press `F5` to run the extension
- open an example file from the folder `example`
- when hovering a hidden code part, the original content gets shown
