...
async function updateDecorations(activeEditor: vscode.TextEditor, languageDrivers: Record<string, LanguageDriver>) {
    ...
    const isEnabled = vscode.workspace.getConfiguration("decorator-hiding").get("enabled");
    ...
    for (let index = 0; index < languageParameters.length; index++) {
        ...
        const leadingCharacters = vscode.workspace.getConfiguration("decorator-hiding").get("leadingCharacters");
        const trailingCharacters = vscode.workspace.getConfiguration("decorator-hiding").get("trailingCharacters");
        const parameterCase = vscode.workspace.getConfiguration("decorator-hiding").get("parameterCase");
    ...
    const codePartsToHide = driver.getCodePartsToHide(code);
    activeEditor.setDecorations(hideDecorationType, codePartsToHide);
}
...
export function activate(context: vscode.ExtensionContext) {
    ...
    vscode.workspace.onDidChangeConfiguration((event) => {
        if (event.affectsConfiguration("decorator-hiding"))
