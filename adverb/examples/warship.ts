...
class Board {
    ...
    constructor(public element: HTMLElement, playerBoard: boolean = true) {
        ...
        var referenceCell = new HTMLElement();
        for (var i = 0; i < this.shipSizes.length; i++) {
            ...
            if (playerBoard) { // Show the ships for positioning.
                ...
                ship.updateLayout();
                (ship.element as any).data("shipIndex", i).draggable({
                    disabled: false,
                    containment: 'parent',
                    // Reduce size slightly to avoid overlap issues blocking the last cell
                    grid: [(referenceCell as any).width() * 0.99 + 2, (referenceCell as any).height() * 0.99 + 2],
                    cursor: 'crosshair'
                }).click((evt) => {
        ...
        (window as any).resize((evt) => {
            (this.element as any).children(".ship").draggable("option", "grid", [(referenceCell as any).width() * 0.99 + 2, (referenceCell as any).height() * 0.99 + 2]);
        });

        if (!playerBoard) {
            // Computer board, this is where the player clicks to bomb
            (element as any).click((evt) => this.onCellClick(evt));
        }
    }

    set dragAndDropEnabled(val: boolean) {
        var cells = (this.element as any).children(".cell");
        var ships = (this.element as any).children(".ship");
    ...
    onCellClick(evt) {
        var x = evt.target;
        if ((x as any).hasClass("cell") === false) {
            return;
        }
        if (!this.playerTurn) {
    ...
    bombCell(cellElem: HTMLElement) {
        var cellPos = Cell.parseCellLocation((cellElem as any).data("cellLocation"));
        ...
        if (cell.shipIndex >= 0) { // Has a ship
            (cellElem as any).removeClass("notBombed");
            (cellElem as any).addClass("cellHit");
            ...
            (cellElem as any).removeClass("notBombed");
            (cellElem as any).addClass("cellMiss");
    ...
    private updateCellData() {
        ...
        (this.element as any).children(".cell").removeClass("cellHit cellMiss").addClass("notBombed");
    }
...
class Game {
    ...
    constructor() {
        ...
        this.playerBoard = new Board(new HTMLElement());
        this.computerBoard = new Board(new HTMLElement(), false);
    ...
    private updateStatus(msg: string) {
        (new HTMLElement as any).slideUp('fast', function () {  // Slide out the old text
            (new HTMLElement as any).text(msg).slideDown('fast');  // Then slide in the new text
        });
    }
}
