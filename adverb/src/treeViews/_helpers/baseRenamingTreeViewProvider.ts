...
export abstract class BaseRenamingTreeViewProvider extends BaseTreeViewProvider<RenamingTreeItem> {
  ...
  async getChildren(element?: RenamingTreeItem): Promise<RenamingTreeItem[]> {
    ...
    else fileConfig = await configuration.getLocalConfiguration(this.uri!);
    if (!fileConfig) return [];

    const renamings = fileConfig.renamings;
    const result: RenamingTreeItem[] = [];
    if (fileConfig.fileRenaming) {
      const renamingType = getRenamingTypes().find(
        (x) => x.id === fileConfig!.fileRenaming
      );
      if (renamingType)
        result.push(
          new RenamingTreeItem(
            "all symbol names",
            renamingType.description,
            fileConfig.fileRenaming
          )
        );
    }
    if (renamings) {
  ...
  async _edit(
    renamingConfiguration: RenamingConfiguration,
    updateRenaming: () => Promise<boolean>
  ) {
    ...
    if (newName) {
      ...
      if (result) {
        ...
        window.showInformationMessage(
          `'${renamingConfiguration.originalName}' successfully updated to '${newName}'.`
        );
      }
    }
  }

  async actuallyRename(
    node: RenamingTreeItem,
    deleteRenaming?: (editor: TextEditor, originalName: string) => Promise<void>
  ) {
    ...
    window
      .showInformationMessage(
        "Do you really want to rename the code symbol in the selected way?",
        ...["Yes", "No"]
      )
      .then(async (answer) => {
        if (answer === "Yes") {
          if (node.originalName === "all symbol names") {
            window.showErrorMessage(
              "Unfortunately, all symbol names cannot be renamed."
            );
            return;
          }
