...
export class RenameAllCommand extends Command {
  ...
  async execute(editor: TextEditor, ...args: any[]) {
    ...
    const items: string[] = renamingTypes
      .filter((x) => x.onlyForSingleRenaming === false)
      .map((x) => x.description);
    const result = await showQuickPick(
      `Choose a renaming technique for all symbols:`,
      items
    );
    ...
    if (renamingType) {
      ...
      if (scope)
        ...
        update = await configuration.updateLocalFileRenaming(
          editor.document.uri,
          renamingType
        );
      if (update) {
