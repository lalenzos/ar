...
export function getParameterName(editor: vscode.TextEditor, position: vscode.Position, key: number, namedValue?: string) {
    return new Promise(async (resolve, reject) => {
        ...
        const shouldHideRedundantAnnotations = vscode.workspace.getConfiguration('decorator').get('hideRedundantAnnotations')
...
function showDollar(str: string): string {
    if (vscode.workspace.getConfiguration('decorator').get('showPhpDollar')) {
        return str
    }
...
function crawlAst(ast, functionCalls = []) {
    ...
    const shouldHideArgumentNames = vscode.workspace.getConfiguration('decorator').get('hideSingleParameters') && ast.arguments && ast.arguments.length === 1
